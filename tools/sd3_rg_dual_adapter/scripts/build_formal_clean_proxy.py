...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build audited Formal5000 Telea clean proxy assets.")
    parser.add_argument("--train-manifest", type=Path, required=True)
    parser.add_argument("--val-manifest", type=Path, required=True)
    parser.add_argument("--out-dir", type=Path, default=Path("formal_assets/clean_proxy"))
    parser.add_argument("--preview-per-class", type=int, default=4)
    args = parser.parse_args()
    qa = build_formal_clean_proxy(args.train_manifest, args.val_manifest, args.out_dir, args.preview_per_class)
    print(f"PROXY_TOTAL={qa['PROXY_TOTAL']}")
    print(f"FORMAL_CLEAN_PROXY_READY={qa['FORMAL_CLEAN_PROXY_READY']}")
    return 0 if qa["FORMAL_CLEAN_PROXY_READY"] == "PASS" else 2


def build_formal_clean_proxy(
    train_manifest: Path,
    val_manifest: Path,
    out_dir: Path,
    ...
    rows = [("train", row) for row in _read_rows(train_manifest)] + [("val", row) for row in _read_rows(val_manifest)]
    qa: dict[str, object] = {
        "PROXY_TOTAL": 0,
        "PROXY_UNIQUE_SOURCE_IMAGES": 0,
        "PROXY_MISSING": 0,
        "PROXY_CORRUPT": 0,
        ...
        "FORMAL_CLEAN_PROXY_READY": "PASS",
    }
    manifest_rows: list[dict[str, str]] = []
    preview_counts: dict[str, int] = {"train_NEG": 0, "val_NEG": 0}
    for split_name in ("train", "val"):
        for cls in ("D00", "D10", "D20", "D40"):
            preview_counts[f"{split_name}_{cls}"] = 0
    source_images: set[str] = set()
    for split_name, row in rows:
        image_path = Path(row["image_path"])
        label_path = Path(row["label_path"])
        source_images.add(row["image_path"])
        ...
        preview_key = f"{split_name}_{'NEG' if row['is_negative'] == 'true' else row['anchor_class']}"
        if preview_counts[preview_key] < preview_per_class:
            _write_preview(preview_dir / f"{split_name}_{row['sample_id']}_{preview_key}.jpg", original, reloaded_mask, reloaded_proxy)
            preview_counts[preview_key] += 1
        manifest_rows.append(
            {
                "sample_id": row["sample_id"],
                "split": split_name,
                "formal_role": "train" if split_name == "train" else "val_full",
    ...
    qa["PROXY_UNIQUE_SOURCE_IMAGES"] = len(source_images)
    qa["FORMAL_CLEAN_PROXY_READY"] = "PASS" if formal_clean_proxy_qa_passed(qa) else "FAIL"
    _write_csv(out_dir / "clean_proxy_manifest.csv", manifest_rows)
    (out_dir / "clean_proxy_audit.json").write_text(json.dumps(qa, indent=2) + "\n", encoding="utf-8")
    return qa


def formal_clean_proxy_qa_passed(qa: dict[str, object], expected_total: int = 2404) -> bool:
    return (
        int(qa.get("PROXY_TOTAL", -1)) == expected_total
        and int(qa.get("PROXY_UNIQUE_SOURCE_IMAGES", -1)) == expected_total
        and int(qa.get("PROXY_MISSING", -1)) == 0
        and int(qa.get("PROXY_CORRUPT", -1)) == 0
        and int(qa.get("PROXY_SHAPE_MISMATCH", -1)) == 0
        and qa.get("NEGATIVE_PROXY_EXACT_MATCH") == "PASS"
        ...
        and qa.get("BLUR_FALLBACK_USED") == "NO"
    )
...
def _write_preview(path: Path, original: Image.Image, mask: Image.Image, proxy: Image.Image) -> None:
    ...
    canvas.paste(mask.convert("RGB"), (width, 0))
